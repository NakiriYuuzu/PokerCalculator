...
var best_index : i32 = 0;`,`if (${i.getByIndices("input_indices")} ${t.selectLastIndex>0?">=":">"} value) {
    ...
    `,iu=(e,t,r,i,a,n,s,u)=>{let d=ve(s?1:n),p=64,h=n/d;h<p&&(p=32);let f=Math.ceil(n/d/p),g=[{type:12,data:t},{type:12,data:r},{type:12,data:i},{type:12,data:a},{type:12,data:h},{type:12,data:f}],y=Te(e.dataType,d),_=Ee(1,d),w=["type"];s&&w.push("type"),u&&w.push("type");let S=x=>{let $=K("x",e.dataType,e.dims,d),I=[$],k=s?N("seq_lens",s.dataType,s.dims):void 0;k&&I.push(k);let E=u?N("total_sequence_length_input",u.dataType,u.dims):void 0;E&&I.push(E);let C=Ee(e.dataType),A=[{name:"batch_size",type:"u32"},{name:"num_heads",type:"u32"},{name:"past_sequence_length",type:"u32"},{name:"sequence_length",type:"u32"},{name:"total_sequence_length",type:"u32"},{name:"elements_per_thread",type:"u32"}];return`
...
`},Ka=(e,t,r,i,a=!1,n)=>{let s=e[0].dims,u=e[1].dims,d=s[s.length-2],p=u[u.length-1],h=s[s.length-1],f=ve(p),g=ve(h),y=ve(d),_=O.size(r)/f/y,w=e.length>2,S=i?i.slice(0,-2):r.slice(0,-2),x=[O.size(S),d,p],$=[{type:12,data:_},{type:12,data:d},{type:12,data:p},{type:12,data:h}];Ct(t,$),$.push(...Y(S,s,u)),w&&$.push(...Y(e[2].dims)),$.push(...Y(x));let I=k=>{let E=La("batch_dims",e[0].dataType,S.length),C=N("a",e[0].dataType,s.length,g),A=N("b",e[1].dataType,u.length,f),v=K("output",e[0].dataType,x.length,f),M=Te(v.type.tensor),D=zt(t,v.type.value,M),H=[C,A],G="";if(w){let j=a?f:1;H.push(N("bias",e[2].dataType,e[2].dims.length,j)),G=`${a?`value += bias[col / ${j}];`:`value += ${v.type.value}(bias[row + i]);`}`}let Q=[{name:"output_size",type:"u32"},{name:"M",type:"u32"},{name:"N",type:"u32"},{name:"K",type:"u32"}];At(t,Q);let R=()=>{let j=`var a_data: ${C.type.value};`;for(let F=0;F<g;F++)j+=`
...
`}return j};return`
  ...
  `};return{name:"MatMulNaive",shaderCache:{hint:`${t.activation};${f};${g};${y};${a}`,inputDependencies:w?["rank","rank","rank"]:["rank","rank"]},getRunData:()=>({outputs:[{dims:n?n(r):r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(_/64)},programUniforms:$}),getShaderSource:I}}}),Su,Tu,xa,qi,ku,Sa,Iu,Fr,Za=q(()=>{te(),ie(),ae(),Rt(),Qa(),ja(),Su=(e,t)=>e?`
  ...
  ${w?xa(S,x,F,j):Sa(S,x,F,j)}
    ...
    fn mm_write(batch: i32, row : i32, colIn : i32, valueIn : ${E}) {
      ...
      fn setOutputAtCoords(d0 : i32, d1 : i32, d2 : i32, d3 : i32, value : ${w?`vec4<${W}>`:W}) {
        ...
        ${w?xa(I,$,W,void 0,!p,v):Sa(I,$,W,void 0,!p,v,!1,void 0,u)}`};return{name:"Conv2DMatMul",shaderCache:{hint:`${t.cacheKey};${E};${w};${M};${D};${H};${C};${A};${v}`,inputDependencies:R},getRunData:()=>({outputs:[{dims:d?d(r):r,dataType:e[0].dataType}],dispatchGroup:{x:k[0],y:k[1],z:k[2]},programUniforms:Q}),getShaderSource:j}}}),zu,Wi,Jt,Cu,Li,Au,Vc,Gc,dy=q(()=>{te(),it(),ie(),ae(),Rt(),ja(),zu=e=>{let t=1;for(let r=0;r<e.length;r++)t*=e[r];return t},Wi=e=>typeof e=="number"?[e,e,e]:e,Jt=(e,t)=>t<=1?e:e+(e-1)*(t-1),Cu=(e,t,r,i=1)=>{let a=Jt(t,i);return Math.floor((e[0]*(r-1)-r+a)/2)},Li=(e,t,r,i,a)=>{a==null&&(a=Cu(e,t[0],i[0]));let n=[0,0,0,r];for(let s=0;s<3;s++)e[s]+2*a>=t[s]&&(n[s]=Math.trunc((e[s]-t[s]+2*a)/i[s]+1));return n},Au=(e,t,r,i,a,n,s,u,d,p)=>{let h,f,g,y;if(e==="VALID"&&(e=0),typeof e=="number"){h={top:e,bottom:e,left:e,right:e,front:e,back:e};let _=Li([t,r,i,1],[u,d,p],1,[a,n,s],e);f=_[0],g=_[1],y=_[2]}else if(Array.isArray(e)){if(!e.every((w,S,x)=>w===x[0]))throw Error(`Unsupported padding parameter: ${e}`);h={top:e[0],bottom:e[1],left:e[2],right:e[3],front:e[4],back:e[5]};let _=Li([t,r,i,1],[u,d,p],1,[a,n,s],e[0]);f=_[0],g=_[1],y=_[2]}else if(e==="SAME_UPPER"){f=Math.ceil(t/a),g=Math.ceil(r/n),y=Math.ceil(i/s);let _=(f-1)*a+u-t,w=(g-1)*n+d-r,S=(y-1)*s+p-i,x=Math.floor(_/2),$=_-x,I=Math.floor(w/2),k=w-I,E=Math.floor(S/2),C=S-E;h={top:I,bottom:k,left:E,right:C,front:x,back:$}}else throw Error(`Unknown padding parameter: ${e}`);return{padInfo:h,outDepth:f,outHeight:g,outWidth:y}},Vc=(e,t,r,i,a,n=!1,s="channelsLast")=>{let u,d,p,h,f;if(s==="channelsLast")[u,d,p,h,f]=e;else if(s==="channelsFirst")[u,f,d,p,h]=e;else throw new Error(`Unknown dataFormat ${s}`);let[g,,y,_,w]=t,[S,x,$]=Wi(r),[I,k,E]=Wi(i),C=Jt(y,I),A=Jt(_,k),v=Jt(w,E),{padInfo:M,outDepth:D,outHeight:H,outWidth:G}=Au(a,d,p,h,S,x,$,C,A,v),Q=n?g*f:g,R=[0,0,0,0,0];return s==="channelsFirst"?R=[u,Q,D,H,G]:s==="channelsLast"&&(R=[u,D,H,G,Q]),{batchSize:u,dataFormat:s,inDepth:d,inHeight:p,inWidth:h,inChannels:f,outDepth:D,outHeight:H,outWidth:G,outChannels:Q,padInfo:M,strideDepth:S,strideHeight:x,strideWidth:$,filterDepth:y,filterHeight:_,filterWidth:w,effectiveFilterDepth:C,effectiveFilterHeight:A,effectiveFilterWidth:v,dilationDepth:I,dilationHeight:k,dilationWidth:E,inShape:e,outShape:R,filterShape:t}},Gc=(e,t,r,i,a,n)=>{let s=n==="channelsLast";s?e[0].dims[3]:e[0].dims[1];let u=[64,1,1],d={x:r.map((S,x)=>x)},p=[Math.ceil(zu(d.x.map(S=>r[S]))/u[0]),1,1];de("verbose",()=>`[conv3d_naive_webgpu] dispatch = ${p}`);let h=1,f=O.size(r),g=[{type:12,data:f},{type:12,data:i},{type:12,data:a},{type:12,data:t.strides},{type:12,data:t.dilations}];Ct(t,g),g.push(...Y(e[0].dims,e[1].dims));let y=["rank","rank"],_=e.length===3;_&&(g.push(...Y(e[2].dims)),y.push("rank")),g.push(...Y(r));let w=S=>{let x=[{name:"output_size",type:"u32"},{name:"filter_dims",type:"u32",length:i.length},{name:"pads",type:"u32",length:a.length},{name:"strides",type:"u32",length:t.strides.length},{name:"dilations",type:"u32",length:t.dilations.length}];At(t,x);let $=1,I=Te(e[0].dataType),k=N("x",e[0].dataType,e[0].dims.length,h),E=N("W",e[1].dataType,e[1].dims.length,$),C=[k,E],A=K("result",e[0].dataType,r.length,$),v="";if(_){let H=N("bias",e[2].dataType,e[2].dims.length,$);C.push(H),v+=`
...
`:"",ud=(e,t,r,i,a)=>{let[n,s,u,d]=r.length===2?[-1,0,1,-1]:[0,2,3,1],p=e.type.value;return`
  ...
  fn bicubicInterpolation(output_indices: ${t.type.indices}) -> ${y} {
    ...
    }`},pd=(e,t,r,i,a,n)=>{let s=e.dims,u=rd(n,t.axes,s.length),d=id(s,i,a,t.axes),p=i.slice();i.length===0&&(p=s.map(($,I)=>$===0?1:d[I]/$),t.keepAspectRatioPolicy!=="stretch"&&(d=ad(s,p,t)));let h=K("output",e.dataType,d.length),f=N("input",e.dataType,s.length),g=O.size(d),y=s.length===d.length&&s.every(($,I)=>$===d[I]),_=t.coordinateTransformMode==="tf_crop_and_resize",w=t.extrapolationValue,S=f.type.value,x=$=>`
      ${y?"":`
...
`}
      ...
      ${E.mainStart([S,1,1])}
        ...
        for (var i: u32 = 0; i < stride; i++) {
          ...
          return input_indices;
      }`,_d=(e,t)=>{let r=e[0].dims,i=O.size(r),a=t.axes.length>0?O.normalizeAxes(t.axes,r.length):[...Array(r.length).keys()],n=rr(e,4);n.forEach(x=>x!==0||(()=>{throw new Error("step cannot be 0")})),n.length===0&&(n=Array(a.length).fill(1));let s=t.starts.map((x,$)=>ua(x,$,r,a,n)),u=t.ends.map((x,$)=>ua(x,$,r,a,n));if(a.length!==s.length||a.length!==u.length)throw new Error("start, ends and axes should have the same number of elements");if(a.length!==r.length)for(let x=0;x<r.length;++x)a.includes(x)||(s.splice(x,0,0),u.splice(x,0,r[x]),n.splice(x,0,1));let d=n.map(x=>Math.sign(x));n.forEach((x,$,I)=>{if(x<0){let k=(u[$]-s[$])/x,E=s[$],C=E+k*n[$];s[$]=C,u[$]=E,I[$]=-x}});let p=r.slice(0);a.forEach((x,$)=>{p[x]=Math.ceil((u[x]-s[x])/n[x])});let h={dims:p,dataType:e[0].dataType},f=K("output",e[0].dataType,p.length),g=N("input",e[0].dataType,e[0].dims.length),y=O.size(p),_=[{name:"outputSize",type:"u32"},{name:"starts",type:"u32",length:s.length},{name:"signs",type:"i32",length:d.length},{name:"steps",type:"u32",length:n.length}],w=[{type:12,data:y},{type:12,data:s},{type:6,data:d},{type:12,data:n},...Y(e[0].dims,p)],S=x=>`
